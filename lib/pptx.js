...
function addObservationsSlide(pres, keyObservations) {
  const slide = pres.addSlide();
...
function addRecommendationsSlide(pres, recommendations) {
  const slide = pres.addSlide();
  addMasthead(slide, "Observations & Recommendations");
...
async function buildSupportSummaryPptx({ participantName, reportPeriod, content }) {
  ...
  addObservationsSlide(pres, content.keyObservations);
  addRecommendationsSlide(pres, content.recommendations);
